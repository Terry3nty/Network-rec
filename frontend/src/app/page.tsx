...
export default function Home() {
  ...
  return (
    <div className="relative flex flex-col justify-center overflow-hidden bg-zinc-950 text-zinc-100 py-20 px-4 sm:px-6 lg:px-8 min-h-[85vh]">
      
      {/* Subtle Warm Orange Glow Spot (Extremely low opacity, no blue/orange gradients) */}
      ...
      <div className="relative z-10 mx-auto max-w-4xl text-center flex flex-col items-center">
        ...
        <motion.p
          ...
          className="mx-auto mt-6 max-w-xl text-base md:text-lg text-zinc-400 leading-relaxed"
        >
        ...
        {/* Main CTA Card (Borderless, Elevation Shadows) */}
        <motion.div
          initial={{ opacity: 0, scale: 0.97 }}
          animate={{ opacity: 1, scale: 1 }}
          transition={{ duration: 0.4, delay: 0.3 }}
          className="mt-10 w-full max-w-md rounded-3xl bg-zinc-900/60 p-6 md:p-8 shadow-2xl backdrop-blur-md border-0"
        >
          {/* GPS Button */}
          <button
            onClick={handleGetLocation}
            disabled={loading}
            className="flex w-full items-center justify-center gap-2.5 rounded-2xl bg-orange-650 py-4.5 px-5 text-base font-bold text-white shadow-lg shadow-orange-650/15 hover:bg-orange-600 focus:outline-none disabled:opacity-75 transition-all group active:scale-[0.98] cursor-pointer border-0"
          >
            {loading ? (
              <>
          ...
          <div className="my-6 flex items-center justify-center gap-3">
            ...
            <span className="text-[10px] font-black uppercase tracking-wider text-zinc-500 shrink-0">or</span>
            <span className="h-px w-full bg-zinc-800/80" />
          </div>
          ...
          <form onSubmit={handleSearchSubmit} className="relative flex gap-2.5">
            <div className="relative flex-grow">
              ...
              <input
                ...
                className="w-full rounded-2xl bg-zinc-950/80 py-3.5 pl-11 pr-4 text-sm text-white placeholder-zinc-500 outline-none focus:ring-1 focus:ring-orange-500/50 transition-all border-0"
              />
            </div>
            <button
              type="submit"
              disabled={searching}
              className="flex h-12 w-12 items-center justify-center rounded-2xl bg-zinc-850 hover:bg-zinc-800 text-zinc-200 hover:text-white border-0 transition-colors cursor-pointer"
            >
              <Search size={18} />
            </button>
          </form>
          ...
          <AnimatePresence>
            {error && (
              <motion.div
                ...
                className="mt-4 text-xs text-red-400 font-bold bg-red-950/20 p-3.5 rounded-xl border-0"
              >
                {error}
              </motion.div>
            )}
          ...
          <AnimatePresence>
            {searchResults.length > 0 && (
              <motion.ul
                ...
                animate={{ opacity: 1, y: 0 }}
                exit={{ opacity: 0 }}
                className="mt-4 overflow-hidden rounded-2xl bg-zinc-900 shadow-2xl divide-y divide-zinc-800/80 text-left max-h-56 overflow-y-auto border-0"
              >
        ...
        <div className="mt-24 grid grid-cols-1 gap-6 sm:grid-cols-3 w-full max-w-4xl text-left">
          <motion.div
            whileHover={{ y: -5 }}
            className="rounded-2xl bg-zinc-900/40 p-6 shadow-lg hover:shadow-xl hover:bg-zinc-900/60 transition-all duration-300 border-0"
          >
            <div className="flex h-10 w-10 items-center justify-center rounded-xl bg-orange-500/10 text-orange-500 mb-4 border-0">
              <Activity size={18} />
            </div>
            <h3 className="font-extrabold text-zinc-100 text-base">Multi-Source Aggregation</h3>
            <p className="mt-2.5 text-xs md:text-sm text-zinc-450 leading-relaxed">
          ...
          >
            ...
            <h3 className="font-extrabold text-zinc-100 text-base">Weighted Scoring</h3>
            <p className="mt-2.5 text-xs md:text-sm text-zinc-450 leading-relaxed">
          ...
          >
            ...
            <h3 className="font-extrabold text-zinc-100 text-base">Zero Bandwidth Cost</h3>
            <p className="mt-2.5 text-xs md:text-sm text-zinc-450 leading-relaxed">
